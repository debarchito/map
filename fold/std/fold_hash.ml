...
module Make(H : Map.Heap.S) = struct
  ...
  let find_slot heap addr k =
    ...
    while not !stop do
      ...
      if is_empty kv then begin
        if !first_tomb >= 0 then found := !first_tomb
        else found := !i mod c;
        stop := true
      end else if is_tomb kv then begin
      ...
      end else if value_equal kv k then begin
        found := !i mod c;
        stop := true
      end else begin
        i := (!i + 1) mod c;
  ...
  let make heap =
    Value.Ptr (alloc_table heap initial_cap)
  ...
  let hash_get heap addr k =
    ...
    if not (is_occupied kv) then Exception.throw e_not_found "hash/get: key not found";
  ...
  let hash_has heap addr k =
    ...
    else is_occupied (H.read heap addr (2 + slot))

  let hash_set heap addr k v =
    let c    = cap heap addr in
    let n    = count heap addr in
    let addr =
      if n * 10 >= c * 7 then rehash heap addr
      else addr
    in
    let c    = cap heap addr in
    let slot = find_slot heap addr k in
  ...
  let register heap (reg : Map.Symbol.registry) =
    NS.register heap reg
      (ns "std/hash" [
        ...
        nif "get_or" (fun args ->
          match args with
          | [| Value.Ptr addr; k; default |] -> hash_get_or heap addr k default
          | _ -> Exception.throw e_bad_arg "hash/get_or expects ptr key default");
        ...
        nif "delete" (fun args ->
          match args with
          | [| Value.Ptr addr; k |] ->
            hash_delete heap addr k; Value.Nil
        ...
        nif "is_hash" (fun args ->
          match args with
          | [| Value.Ptr addr |] ->
            Value.Int (if H.get_tag heap addr = Fold_core.Tag.hash then 1 else 0)
          | [| _ |] -> Value.Int 0
