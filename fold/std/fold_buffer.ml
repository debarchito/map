...
module Make (H : Map.Heap.S) = struct
  ...
  let write_byte heap addr i byte =
    ...
    let off = i mod bytes_per_slot in
    let old =
      match H.read heap addr (2 + slot) with Value.Int v -> v | _ -> 0
    in
         ...
         [
               ...
               | [| Value.Int cap |] ->
                   if cap < 0 then
                     Exception.throw e_bad_arg
                       "buffer/make: capacity must be >= 0";
           ...
           nif "set_length" (fun args ->
               ...
               | [| Value.Ptr addr; Value.Int n |] ->
                   ...
                   Value.Nil
               | _ ->
                   Exception.throw e_bad_arg "buffer/set_length expects ptr int");
           nif "read_byte" (fun args ->
               match args with
               | [| Value.Ptr addr; Value.Int i |] ->
                   Value.Int (read_byte heap addr i)
               | _ ->
                   Exception.throw e_bad_arg "buffer/read_byte expects ptr int");
           nif "write_byte" (fun args ->
               ...
               | _ ->
                   Exception.throw e_bad_arg
                     "buffer/write_byte expects ptr int int");
           nif "copy" (fun args ->
               match args with
               | [|
                Value.Ptr src;
           ...
           nif "compare" (fun args ->
               ...
               |] ->
                   if len < 0 then
                     Exception.throw e_bad_arg
                       "buffer/compare: length must be >= 0";
