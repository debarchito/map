    ...
    NS.register heap reg
      (ns "std/cons" [
        ...
        nif "set_car" (fun args ->
          match args with
          | [| Value.Ptr addr; v |] -> set_car heap addr v; Value.Nil
          | _ -> Exception.throw e_bad_arg "cons/set_car expects ptr value");

        nif "set_cdr" (fun args ->
          match args with
          | [| Value.Ptr addr; v |] -> set_cdr heap addr v; Value.Nil
          | _ -> Exception.throw e_bad_arg "cons/set_cdr expects ptr value");

        nif "is_cons" (fun args ->
          match args with
          | [| Value.Ptr addr |] ->
            Value.Int (if H.get_tag heap addr = Fold_core.Tag.cons then 1 else 0)
          | [| _ |] -> Value.Int 0
