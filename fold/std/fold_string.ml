...
module Make(H : Map.Heap.S) = struct
  ...
  let to_ocaml heap addr =
    match H.read heap addr 0,
          H.read heap addr 1,
          H.read heap addr 3 with
      ...
      (ns "std/string" [
        ...
        nif "byte_len" (fun args ->
          ...
          | [| Value.Ptr addr |] ->
            (match H.read heap addr 1 with
             | Value.Int n -> Value.Int n
             | _ -> Exception.throw e_bad_object "string/byte_len: bad string")
        ...
        nif "concat" (fun args ->
          ...
          | [| a; b |] ->
            ...
            alloc_string heap
              ~offset:0 ~byte_length:total ~char_length:char_len ~buf_addr
          | _ -> Exception.throw e_bad_arg "string/concat expects 2 strings");
          ...
          | [| Value.Ptr addr; Value.Int lo; Value.Int hi |] ->
            (match H.read heap addr 0,
             ...
             | Value.Int offset, Value.Int byte_len, Value.Ptr buf_addr ->
               ...
               let len = hi - lo in
               if len < 0 then Exception.throw e_bounds "string/slice: bad range";
               let raw      = Buf.blit_to_bytes heap buf_addr (offset + lo) len in
               ...
               alloc_string heap
                 ...
                 ~char_length:char_len
                 ~buf_addr
             | _ -> Exception.throw e_bad_object "string/slice: bad string")
          | _ -> Exception.throw e_bad_arg "string/slice expects ptr int int");
            ...
            (match H.read heap addr 0,
             ...
             | Value.Int offset, Value.Int byte_len, Value.Ptr buf_addr ->
               if i < 0 || i >= byte_len then
                 Exception.throw e_bounds "string/byte_set: out of bounds";
               Buf.write_byte heap buf_addr (offset + i) (byte land 0xFF);
               Value.Nil
             | _ -> Exception.throw e_bad_object "string/byte_set: bad string")
          | _ -> Exception.throw e_bad_arg "string/byte_set expects ptr int int");

        nif "eq" (fun args ->
          match args with
          | [| a; b |] ->
            Value.Int (if estr a = estr b then 1 else 0)
          | _ -> Exception.throw e_bad_arg "string/eq expects 2 strings");

        nif "compare" (fun args ->
          match args with
          | [| a; b |] ->
            Value.Int (String.compare (estr a) (estr b))
          | _ -> Exception.throw e_bad_arg "string/compare expects 2 strings");
        ...
        nif "from_float" (fun args ->
          ...
          | _ -> Exception.throw e_bad_arg "string/from_float expects float");

        nif "from_float_fmt" (fun args ->
          match args with
          | [| Value.Float f |] -> str (Printf.sprintf "%g" f)
          | _ -> Exception.throw e_bad_arg "string/from_float_fmt expects float");
        ...
        nif "to_float" (fun args ->
          ...
          | [| s |] ->
            (match float_of_string_opt (estr s) with
             ...
             | None   -> Exception.throw e_parse "string/to_float: not a float")
        ...
        nif "contains" (fun args ->
          ...
          | _ -> Exception.throw e_bad_arg "string/contains expects 2 strings");
        ...
        nif "split" (fun args ->
          ...
          | [| s; sep |] ->
            ...
            let addr  = H.alloc heap ~size:(n + 1) ~tag:Fold_core.Tag.vector in
            H.write heap addr 0 (Value.Int n);
            List.iteri (fun i p ->
              H.write heap addr (1 + i) (of_ocaml heap p)
            ) parts;
            Value.Ptr addr
          | _ -> Exception.throw e_bad_arg "string/split expects string sep");

        nif "join" (fun args ->
          match args with
          | [| Value.Ptr arr; sep |] ->
            let sep = estr sep in
            let n   = match H.read heap arr 0 with
                      | Value.Int n -> n
                      | _ -> Exception.throw e_bad_object "string/join: expected vector" in
