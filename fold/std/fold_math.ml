         ...
         nif "abs" (fun args ->
             ...
             | _ ->
                 Exception.throw e_arity
                   (Printf.sprintf "math/abs expects 1 argument, got %d"
                      (Array.length args)));
         nif "sqrt" (fun args ->
             match args with
             ...
             | _ ->
                 Exception.throw e_arity
                   (Printf.sprintf "math/sqrt expects 1 argument, got %d"
                      (Array.length args)));
         nif "sin" (fun args ->
             match args with
             ...
             | _ ->
                 Exception.throw e_arity
                   (Printf.sprintf "math/sin expects 1 argument, got %d"
                      (Array.length args)));
         nif "cos" (fun args ->
             match args with
             ...
             | _ ->
                 Exception.throw e_arity
                   (Printf.sprintf "math/cos expects 1 argument, got %d"
                      (Array.length args)));
         nif "atan2" (fun args ->
             match args with
             ...
             | _ ->
                 Exception.throw e_arity
                   (Printf.sprintf "math/atan2 expects 2 arguments, got %d"
                      (Array.length args)));
         nif "lerp" (fun args ->
             match args with
             ...
             | _ ->
                 Exception.throw e_arity
                   (Printf.sprintf "math/lerp expects 3 arguments, got %d"
                      (Array.length args)));
         nif "clamp" (fun args ->
             match args with
             ...
             | [| _; _; _ |] ->
                 Exception.throw e_bad_arg
                   "math/clamp expects float float float"
             | _ ->
                 Exception.throw e_arity
                   (Printf.sprintf "math/clamp expects 3 arguments, got %d"
                      (Array.length args)));
         nif "map" (fun args ->
             match args with
             | [|
             ...
             | _ ->
                 Exception.throw e_arity
                   (Printf.sprintf "math/map expects 5 arguments, got %d"
                      (Array.length args)));
         ns "vec2"
           [
             nif "mag" (fun args ->
                 ...
                 | _ ->
                     Exception.throw e_arity
                       (Printf.sprintf "vec2/mag expects 2 arguments, got %d"
                          (Array.length args)));
             nif "dist" (fun args ->
                 match args with
                 | [|
                 ...
                 | _ ->
                     Exception.throw e_arity
                       (Printf.sprintf "vec2/dist expects 4 arguments, got %d"
                          (Array.length args)));
             nif "dot" (fun args ->
                 match args with
                 | [|
                 ...
                 | _ ->
                     Exception.throw e_arity
                       (Printf.sprintf "vec2/dot expects 4 arguments, got %d"
                          (Array.length args)));
           ];
         ns "rng"
           [
