...
module Make(H : Map.Heap.S) = struct
  ...
  let get heap addr i =
    ...
    if i < 0 || i >= n then
      Exception.throw e_bounds (Printf.sprintf "record/get: field %d out of bounds (n %d)" i n);
    H.read heap addr (2 + i)

  let set heap addr i v =
    let n = nfields heap addr in
    if i < 0 || i >= n then
      Exception.throw e_bounds (Printf.sprintf "record/set: field %d out of bounds (n %d)" i n);
    ...
    NS.register heap reg
      (ns "std/record" [
        ...
        nif "schema" (fun args ->
          ...
          | [| Value.Ptr addr |] ->
            (match H.read heap addr 0 with
             ...
             | _ -> Exception.throw e_bad_object "record/schema: bad object")
        ...
        nif "set" (fun args ->
          ...
          | _ -> Exception.throw e_bad_arg "record/set expects ptr int value");

        nif "is_record" (fun args ->
          match args with
          | [| Value.Ptr addr |] ->
            Value.Int (if H.get_tag heap addr = Fold_core.Tag.record then 1 else 0)
          | [| _ |] -> Value.Int 0
        ...
        nif "is_schema" (fun args ->
          ...
          | [| Value.Ptr addr; Value.Int s |] ->
            (match H.read heap addr 0 with
             ...
             | _ -> Exception.throw e_bad_object "record/is_schema: bad object")
          | _ -> Exception.throw e_bad_arg "record/is_schema expects ptr int");
      ])
