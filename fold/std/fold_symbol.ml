...
module Make(H : Map.Heap.S) = struct
  ...
  let get_table () = match !table_ref with
    | Value.Ptr addr -> addr
  ...
  let name_of heap addr =
    ...
    | Value.Ptr _ as v -> Str.to_ocaml heap (match v with Value.Ptr a -> a | _ -> assert false)
    | _ -> Exception.throw e_bad_object "symbol/name_of: bad object"

  let name_val_of heap addr =
    H.read heap addr 1
    ...
    NS.register heap reg
      (ns "std/symbol" [
        ...
        nif "eq" (fun args ->
          ...
          | _ -> Exception.throw e_bad_arg "symbol/eq expects 2 symbol ptrs");
