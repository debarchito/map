...
module Make (H : Map.Heap.S) = struct
  ...
  let get heap addr i =
    ...
    if i < 0 || i >= n then
      Exception.throw e_bounds
        (Printf.sprintf "vector/get: index %d out of bounds (len %d)" i n);
  ...
  let set heap addr i v =
    ...
    if i < 0 || i >= n then
      Exception.throw e_bounds
        (Printf.sprintf "vector/set: index %d out of bounds (len %d)" i n);
         ...
         [
           ...
           nif "set" (fun args ->
               ...
               | [| Value.Ptr addr; Value.Int i; v |] ->
                   ...
                   Value.Nil
               | _ ->
                   Exception.throw e_bad_arg "vector/set expects ptr int value");
           nif "is_vector" (fun args ->
               match args with
               | [| Value.Ptr addr |] ->
                   Value.Int
                     (if H.get_tag heap addr = Fold_core.Tag.vector then 1
                      else 0)
               | [| _ |] -> Value.Int 0
