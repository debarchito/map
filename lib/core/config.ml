...
let validate t =
  ...
  if t.major_threshold     <= 0   then failwith "Config: major_threshold must be > 0";
  if t.major_growth_factor <= 1.0 then failwith "Config: major_growth_factor must be > 1.0"
